...
export function AuthProvider({children}: AuthProviderProps) {
  ...
  async function refresh() {
    try {
      const response: AxiosResponse<LoginResponse> = await api.get('auth/refresh', {headers: {Authorization: `Bearer ${refreshToken}`}})
  ...
  async function login({email, password}:LoginCredentials) {
    ...
    try {
      const respose:AxiosResponse<LoginResponse> = await api.post('auth/login')
  ...
  return (
    <AuthContext.Provider value={{ login, isAuthenticated, accessToken, refreshToken, refresh }}>
      {children}
